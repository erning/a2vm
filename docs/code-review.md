...
| # | 项目 | 状态 |
|---|------|------|
| 1 | `run_cycles` fast-disk 路径未调用 `disk.tick()` | ✅ 已完成 |
| 2 | `read_operand` / `addr_of` 静默返回 0 | ✅ 已完成 |
| 3 | TUI 与 GUI 大量重复代码 | ⏸️ 暂缓（按当前决策暂停） |
| 4 | 缺少自定义错误类型 | ✅ 已完成 |
| 5 | I/O 地址匹配依赖顺序 | ✅ 已完成 |
| 6 | ILL（非法操作码）静默忽略 | 🟡 部分完成（已加 debug 日志，未实现常用非法操作码） |
| 7 | Disk II 不支持写入 | 🟡 部分完成（已支持 RWTS 写入路径，完整写回链路仍可继续完善） |
| 8 | 缺少反汇编器 | ✅ 已完成 |
| 9 | 缺少 CPU 指令级单元测试 | ✅ 已完成 |
| 10 | 集成测试依赖外部 ROM 文件 | ✅ 已完成 |
| 11 | HGR 颜色渲染重复计算行地址 | ✅ 已完成 |
| 12 | `render_until` 每次调用分配新 Vec | 🟡 部分完成（已提供可复用缓冲 API） |
| 13 | `pixels.render().ok()` 静默吞噬错误 | ✅ 已完成 |
| 14 | `Status` 标志位类型安全性 | ✅ 已完成 |
| 15 | `Cpu` 所有字段 `pub` | ⬜ 未处理 |
| 16 | `disk_controller_enabled` 默认值 | ✅ 已完成 |
| 17 | `CHAR_ROM` 硬编码与来源说明 | ✅ 已完成（已补充来源说明） |
| 18 | `architecture.md` 与代码不同步 | ✅ 已完成 |
| 19 | `milestones.md` 未更新 | ✅ 已完成 |
| 20 | `encode_6and2` 魔法数字较多 | ✅ 已完成 |
